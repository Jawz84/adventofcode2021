...
for draw in numbers_to_draw:
    ...
    # if there is a card that has bingo, get card that has bingo
    for card in bingo_cards:
        if has_bingo(card):
            print('Bingo!')
            print('draw: ' + str(draw))
            print('sum: ' + str(sum_non_negative_values(card)))
            print('draw x sum: ' + str(sum_non_negative_values(card) * draw))
            exit()
